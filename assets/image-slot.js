  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      ...
      if (!file || ACCEPT.indexOf(file.type) < 0) {
        this._setError('Drop a PNG, JPEG, GIF, WebP, or AVIF image.');
        return;
      }
      ...
      try {
        ...
        // GIF는 재인코딩 시 애니메이션이 사라지므로 원본 dataURL 그대로 사용
        const url = file.type === 'image/gif'
          ? await new Promise((res, rej) => { const fr = new FileReader(); fr.onload = () => res(fr.result); fr.onerror = rej; fr.readAsDataURL(file); })
